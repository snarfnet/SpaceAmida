...
def remove_review_submission_items(submission_id):
    r = api('GET', f'/reviewSubmissions/{submission_id}/items?limit=50')
    if r.status_code != 200:
        print(f'Could not list items for reviewSubmission {submission_id}: {r.status_code} {short_error(r)}')
        return False

    items = r.json().get('data') or []
    print(f'Removing {len(items)} item(s) from reviewSubmission: {submission_id}')
    ok = True
    for item in items:
        item_id = item.get('id')
        if not item_id:
...
def add_review_submission_item(submission_id, version_id):
    return api('POST', '/reviewSubmissionItems', json={
        'data': {
...
def submit_review_submission(version_id):
    ...
    if r.status_code not in (200, 201):
        ...
        if existing:
            ...
            if not remove_review_submission_items(existing_id):
                return False, f'Could not remove stale reviewSubmissionItems from {existing_id}'
            time.sleep(10)
            r = add_review_submission_item(submission_id, version_id)
            if r.status_code not in (200, 201):
                return False, f'Add reviewSubmissionItem failed after cleanup: {r.status_code} {short_error(r)}'
